const Registry = function(req,res,connection){
    ...
    let querySqls=`SELECT id,name,account,password,avatar FROM user_info WHERE account=${param.account};`
    connection.query(querySqls,(err,rst,fields)=>{
        if(err){
            ...
            if(result.length>0){
                ...
                let insertSqls=`INSERT INTO user_info(id,name,account,password,avatar) VALUES(?,?,?,?,?)`
